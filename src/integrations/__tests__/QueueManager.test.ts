...
describe("QueueManager", () => {
  ...
  const mockTrack1: PlayableTrack = { id: "1", title: "Track 1", url: "http://example.com/track1", source: "youtube", duration: 180 };
  const mockTrack2: PlayableTrack = { id: "2", title: "Track 2", url: "http://example.com/track2", source: "soundcloud", duration: 240 };
  const mockTrack3: PlayableTrack = { id: "3", title: "Track 3", url: "http://example.com/track3", source: "youtube", duration: 200 };

  beforeEach(() => {
    queueManager = new QueueManager({ maxSize: 5 });
  });
  ...
  test("should not exceed max size", () => {
    const tracks = Array(6).fill(0).map((_, i) => ({ ...mockTrack1, id: `t${i}`, title: `Track ${i}` }));
  ...
  test("should set and get loop mode", () => {
    queueManager.setLoopMode(LoopMode.TRACK);
    expect(queueManager.getLoopMode()).toBe(LoopMode.TRACK);
    queueManager.setLoopMode(LoopMode.QUEUE);
    expect(queueManager.getLoopMode()).toBe(LoopMode.QUEUE);
    queueManager.setLoopMode(LoopMode.NONE);
    expect(queueManager.getLoopMode()).toBe(LoopMode.NONE);
  });
